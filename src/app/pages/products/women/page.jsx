...
export default function Page() {
  ...
  const filterByCategory = (product) => {
    return product.category.toLowerCase().trim() === "women";
  };
  ...
  const womenFilter = (products) => {
    const filteredItems = products.filter((product) => {
      let categoryFilter = filterByCategory(product);
      let typeFilter = true;
      let sizeFilter = true;
  ...
  const handleFilterButtonClick = () => {
    setFilteredProducts(womenFilter(products));
  ...
  useEffect(() => {
    api
      .get("/Product")
      .then((response) => {
        setProducts(womenFilter(response.data));
        const uniqueCategories = [
          ...new Set(womenFilter(response.data).map((product) => product.category)),
        ];
        const uniqueTypes = [
          ...new Set(womenFilter(response.data).map((product) => product.type)),
        ];
        const uniqueColors = [
          ...new Set(womenFilter(response.data).flatMap((product) => product.color)),
        ];
        const uniqueSizes = [
          ...new Set(womenFilter(response.data).flatMap((product) => product.sizes)),
        ];

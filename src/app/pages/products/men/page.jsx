...
export default function Page() {
  ...
  const handleFilterButtonClick = () => {
    setFilteredProducts(menFilter(products));
  ...
  useEffect(() => {
    api
      .get("/Product")
      .then((response) => {
        setProducts(menFilter(response.data));
        const uniqueCategories = [
          ...new Set(menFilter(response.data).map((product) => product.category)),
        ];
        const uniqueTypes = [
          ...new Set(menFilter(response.data).map((product) => product.type)),
        ];
        const uniqueColors = [
          ...new Set(menFilter(response.data).flatMap((product) => product.color)),
        ];
        const uniqueSizes = [
          ...new Set(menFilter(response.data).flatMap((product) => product.sizes)),
        ];

...
export default function Page() {
  ...
  const filterByCategory = (product) => {
    return product.category.toLowerCase().trim() === "girl";
  };
  ...
  const handleFilterButtonClick = () => {
    setFilteredProducts(girlFilter(products));
  ...
  useEffect(() => {
    api
      .get("/Product")
      .then((response) => {
        setProducts(girlFilter(response.data));
        const uniqueCategories = [
          ...new Set(girlFilter(response.data).map((product) => product.category)),
        ];
        const uniqueTypes = [
          ...new Set(girlFilter(response.data).map((product) => product.type)),
        ];
        const uniqueColors = [
          ...new Set(girlFilter(response.data).flatMap((product) => product.color)),
        ];
        const uniqueSizes = [
          ...new Set(girlFilter(response.data).flatMap((product) => product.sizes)),
        ];

...
export default function Page() {
  ...
  const filterByCategory = (product) => {
    return product.category.toLowerCase().trim() === "boy";
  };
  ...
  const handleFilterButtonClick = () => {
    setFilteredProducts(boyFilter(products));
  ...
  useEffect(() => {
    api
      .get("/Product")
      .then((response) => {
        setProducts(boyFilter(response.data));
        const uniqueCategories = [
          ...new Set(boyFilter(response.data).map((product) => product.category)),
        ];
        const uniqueTypes = [
          ...new Set(boyFilter(response.data).map((product) => product.type)),
        ];
        const uniqueColors = [
          ...new Set(boyFilter(response.data).flatMap((product) => product.color)),
        ];
        const uniqueSizes = [
          ...new Set(boyFilter(response.data).flatMap((product) => product.sizes)),
        ];
